...
public:
	...
	//////////////////////////////////////////////////////////////////
	BOOL MoveToFront(POSITION pos);
	BOOL MoveToRear(POSITION pos);
...
protected:	
	...
	BOOL __Unlink(LPNODE pTarget, BOOL bDestroy = TRUE); // Unlink and delete a node.
...
template <class TYPE, class ARG_TYPE, void (CALLBACK *FREE_TYPE)(TYPE&)>
BOOL CLinkedList<TYPE, ARG_TYPE, FREE_TYPE>::__Unlink(LPNODE pTarget, BOOL bDestroy)
{
	ASSERT(pTarget != NULL && m_nItemCount > 0);

	// Invoke the callback function
	if (bDestroy && FREE_TYPE != NULL)
...
template <class TYPE, class ARG_TYPE, void (CALLBACK *FREE_TYPE)(TYPE&)>
BOOL CLinkedList<TYPE, ARG_TYPE, FREE_TYPE>::MoveToRear(POSITION pos)
{
	if (m_nItemCount < 2)
		return TRUE;

	LPNODE pTarget = (LPNODE)pos;

	if (!__IsValidDataNode(pTarget))
		return FALSE;

	__Unlink(pTarget, FALSE);
	__Link(m_pTail->pPrev, pTarget, m_pTail);

	return TRUE;
}

template <class TYPE, class ARG_TYPE, void (CALLBACK *FREE_TYPE)(TYPE&)>
BOOL CLinkedList<TYPE, ARG_TYPE, FREE_TYPE>::MoveToFront(POSITION pos)
{
	if (m_nItemCount < 2)
		return TRUE;
	...
	__Unlink(pTarget, FALSE);
	__Link(m_pHead, pTarget, m_pHead->pNext);

	return TRUE;
}

template <class TYPE, class ARG_TYPE, void (CALLBACK *FREE_TYPE)(TYPE&)>
int CLinkedList<TYPE, ARG_TYPE, FREE_TYPE>::GetItemCount() const
{
	return m_nItemCount;
}

template <class TYPE, class ARG_TYPE, void (CALLBACK *FREE_TYPE)(TYPE&)>
BOOL CLinkedList<TYPE, ARG_TYPE, FREE_TYPE>::IsEmpty() const
{
