...
//
// 7/08/2002    Abin (abinn32@yahoo.com)
//
// Update history:
//
...
// 10/30/2004 -- Added "EnumSections" and "EnumKeys"
//
///////////////////////////////////////////////////////////////////
...
BOOL CIni::TrimString(LPTSTR lpString)
{
	if (lpString == NULL)
		return FALSE;
...
BOOL CIni::EnumSections(fnEnumSectionsProc lpfn, LPVOID lpParam) const
{
	if (lpfn == NULL)
		return FALSE;
	...
	BOOL bOK = TRUE;
	for (LPCTSTR pSection = psz; pSection[0]; pSection = &pSection[_tcslen(pSection) + 1])
	{
		bOK = lpfn(pSection, lpParam);
		if (!bOK)
			break;
	}
...
BOOL CIni::EnumKeys(LPCTSTR lpSection, fnEnumKeysProc lpfn, LPVOID lpParam) const
{
	if (lpfn == NULL)
		return FALSE;
	...
	BOOL bOK = TRUE;
	for (LPCTSTR pLine = psz; pLine[0]; pLine = &pLine[_tcslen(pLine) + 1])
	{
		LPTSTR pKey = _tcsdup(pLine);
		LPTSTR pValue = _tcschr(pKey, _T('='));

		if (pValue == NULL)
		{
			bOK = lpfn(pKey, _T(""), lpParam);
		}
		else
		{
			*pValue = _T('\0');
