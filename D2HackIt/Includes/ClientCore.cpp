...
//	BIND_AND_CHECK(FreeHackProfileString);		// 0.62 This will be obsoleted. Don't use this after 0.7!
	...
	BIND_AND_CHECK(PatchFill);					// 2.00 (Abin)
	...
	BIND_AND_CHECK(GetMonsterAttributes);		// 2.00 (Abin)
	...
	BIND_AND_CHECK(CalculateTeleportPath);		// 2.00 (Abin)
	...
	BIND_AND_CHECK(IsQuestItem);				// 2.00 (Abin)
	BIND_AND_CHECK(GetItemQuality);				// 2.00 (Abin)
	BIND_AND_CHECK(GetItemPrefixSuffix);		// 2.00 (Abin)
	BIND_AND_CHECK(GetPrefixSuffixName);		// 2.00 (Abin)
	BIND_AND_CHECK(GetScreenSize);				// 2.00 (Abin)
	BIND_AND_CHECK(ScreenToMapCoords);			// 2.00 (Abin)
	BIND_AND_CHECK(MapToScreenCoords);			// 2.00 (Abin)
	BIND_AND_CHECK(GetMousePosition);			// 2.00 (Abin)
	BIND_AND_CHECK(GetRandomPosition);			// 2.00 (Abin)
	BIND_AND_CHECK(GetAngle);					// 2.00 (Abin)
	BIND_AND_CHECK(CreateCollisionMap);			// 2.00 (Abin)
	BIND_AND_CHECK(GetCollisionData);			// 2.00 (Abin)
	BIND_AND_CHECK(CalculateWalkPath);			// 2.00 (ustc_tweeg)
	BIND_AND_CHECK(DumpCollisionMap);			// 2.00 (Abin)
	BIND_AND_CHECK(GetItemPrice);				// 2.00 (Abin)
	BIND_AND_CHECK(GetItemAttributes);			// 2.00 (Abin)
	BIND_AND_CHECK(GetItemSockets);				// 2.00 (Abin)
	BIND_AND_CHECK(UnitOverhead);				// 2.00 (Abin)
	BIND_AND_CHECK(GetItemProperties);			// 2.00 (Abin)
	BIND_AND_CHECK(FindPresetUnit);				// 2.00 (Abin)
	BIND_AND_CHECK(GetHighlightUnit);			// 2.00 (Abin)
	BIND_AND_CHECK(GetUnitMode);				// 2.00 (Abin)
	BIND_AND_CHECK(EnumGroundItems);			// 2.00 (Abin)
	BIND_AND_CHECK(ExamItemProperties);			// 2.00 (Abin)
	BIND_AND_CHECK(GetItemLevel);				// 2.00 (Abin)
	BIND_AND_CHECK(GetItemDescription);			// 2.00 (Abin)
	BIND_AND_CHECK(GetMapRect);					// 2.00 (Abin)
	BIND_AND_CHECK(GetD2HackitVersion);			// 2.00 (Abin)
	BIND_AND_CHECK(GetD2HackitBuild);			// 2.00 (Abin)
	BIND_AND_CHECK(LoadD2Bitmap);				// 2.00 (Abin)
	BIND_AND_CHECK(DestroyD2Bitmap);			// 2.00 (Abin)
	BIND_AND_CHECK(GetUnitBaseStat);			// 2.00 (Abin)
	BIND_AND_CHECK(GetItemLevelRequirement);	// 2.00 (Abin)
	BIND_AND_CHECK(GetItemStrRequirement);		// 2.00 (Abin)
	BIND_AND_CHECK(GetItemDexRequirement);		// 2.00 (Abin)
	BIND_AND_CHECK(GetItemDamage);				// 2.00 (Abin)
	BIND_AND_CHECK(IsTwoHandItem);				// 2.00 (Abin)
	BIND_AND_CHECK(GetItemSize2);				// 2.00 (Abin)
	BIND_AND_CHECK(GetItemType);				// 2.00 (Abin)
	BIND_AND_CHECK(IsObjectDoor);				// 2.00 (Abin)
	BIND_AND_CHECK(IsObjectChest);				// 2.00 (Abin)
	BIND_AND_CHECK(IsObjectShrine);				// 2.00 (Abin)
	BIND_AND_CHECK(GetItemLocationType);		// 2.00 (Abin)
	BIND_AND_CHECK(GetItemProperties2);			// 2.00 (Abin)
	BIND_AND_CHECK(GetItemImageIndex);			// 1215 (Abin)
	...
	BIND_TO_PLAYER(GetAvailableBeltSlots);		// 2.00 (Abin)
	BIND_TO_PLAYER(GetCurrentAct);				// 2.00 (Abin)
	BIND_TO_PLAYER(GetStorageItem);				// 2.00 (Abin)
	BIND_TO_PLAYER(SellInventoryItems);			// 2.00 (Abin)
	BIND_TO_PLAYER(GambleAmulets);				// 2.00 (Abin)
	BIND_TO_PLAYER(GambleRings);				// 2.00 (Abin)
	BIND_TO_PLAYER(GambleCirclets);				// 2.00 (Abin)
	BIND_TO_PLAYER(GambleGloves);				// 2.00 (Abin)
	BIND_TO_PLAYER(GambleBoots);				// 2.00 (Abin)
	BIND_TO_PLAYER(GambleBelts);				// 2.00 (Abin)
	BIND_TO_PLAYER(IsNpcSessionStarted);		// 2.00 (Abin)
	BIND_TO_PLAYER(IsWeaponSwitched);			// 2.00 (Abin)	
	BIND_TO_PLAYER(GetAngleFrom);				// 2.00 (Abin)
	BIND_TO_PLAYER(PutInventoryPotionToBelt);	// 2.00 (Abin)
	BIND_TO_PLAYER(GetClassID);					// 2.00 (Abin)
	BIND_TO_PLAYER(GetPvpFlags);				// 2.00 (Abin)
	BIND_TO_PLAYER(GetBaseStat);				// 2.00 (Abin)
	BIND_TO_PLAYER(GetMercEquip);				// 1216 (Abin)
