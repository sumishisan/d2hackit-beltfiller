...
typedef int (__cdecl *fnGetItemLocationType)(DWORD dwItemID);
typedef DWORD (__cdecl *fnGetItemProperties2)(DWORD dwItemID, LPITEMMOD lpBuffer, DWORD dwMaxCount);
typedef int (__cdecl *fnGetItemImageIndex)(DWORD dwItemID);
...
//	fnGetInfoStruct					GetInfoStruct;
	...
	fnGetItemLocationType			GetItemLocationType;	// 2.00 (Abin)
	fnGetItemProperties2			GetItemProperties2;		// 2.00 (Abin)
	fnGetItemImageIndex				GetItemImageIndex;		// 1215 (Abin)
...
typedef DWORD (__cdecl *fnGetMercEquip)(int nEquipLocation); // 1216
...
typedef struct tagPlayerEntryPoints
{
	...
	fnGetMercEquip					GetMercEquip;			// 1216 (Abin)
